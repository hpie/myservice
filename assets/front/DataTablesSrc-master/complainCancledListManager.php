...
$statusArray=array('CANCLED'); 
$status = join("','",$statusArray); 

$where =" ticket_status IN('$status') AND sa.appointment_status='CANCLED' ";
$year=$_REQUEST['year'];
...
echo json_encode(
    SSP::complainCancledListManager($_REQUEST, $sql_details, $table, $primaryKey, $columns,$where)
);
