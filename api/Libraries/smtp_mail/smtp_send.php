...
//                            $url='http://google.com';
    ...
    public function send_email_Confirmation($email, $id, $name) {
        $this->subject = "Email Confirmation : " . $this->app_name . " Application";
        $this->link = "http://web.socialinfotech.com/taskaway/emailverify/index.php?datas=";
        $this->mail->isSMTP();
        //Enable SMTP debugging
        // 0 = off (for production use)
        // 1 = client messages
...
//                            $url='http://google.com';
        ...
        //print_r($newurl);die;
        $word = array('{{name}}', '{{action_url}}');
        $replace = array($name, $newurl);
        $html = str_replace($word, $replace, $html);
